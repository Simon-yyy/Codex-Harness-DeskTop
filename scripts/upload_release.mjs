...
async function fetchWithRetry(url, options, retries = 5, delayMs = 3000) {
  for (let i = 0; i < retries; i++) {
    try {
      ...
      print(`[重试 ${i + 1}/${retries}] 网络请求超时或异常，${delayMs / 1000}s 后重试... (${err.message})`);
...
async function upload() {
  ...
  if (!relRes.ok) {
    if (relRes.status === 404) {
      ...
      print(`✓ 成功创建 Release: ${release.name} (ID: ${release.id})`);
    } else {
      throw new Error(`获取 Release 失败: ${relRes.status} ${await relRes.text()}`);
    }
  } else {
    release = await relRes.json();
    print(`✓ 找到 Release: ${release.name} (ID: ${release.id})`);
    const notesPath = path.join(path.resolve('release', TAG), 'RELEASE_NOTES.md');
    if (fs.existsSync(notesPath)) {
      const body = fs.readFileSync(notesPath, 'utf8');
      await fetchWithRetry(`https://api.github.com/repos/${OWNER}/${REPO}/releases/${release.id}`, {
        method: 'PATCH',
        headers,
        body: JSON.stringify({ body })
      });
      print('✓ Release 页面说明已成功同步刷新！');
    }
  }
  print(`   已有资产数: ${release.assets ? release.assets.length : 0}`);
  ...
  for (const item of filesToUpload) {
    ...
    print(`2. 上传资产: ${item.name} (${(stat.size / 1024 / 1024).toFixed(2)} MB)...`);
    ...
    print(`   ✓ 上传成功: ${assetObj.name} (ID: ${assetObj.id})`);
  }
